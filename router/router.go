...
func RouterInit(c *controller.AppControllerImpl, m *middleware.AppMiddlewareImpl) {
	...
	api := router.Group("/api")
	{
		...
		gas := api.Group("/gas")
		gas.GET("", m.RoleMid.IsLoggedIn, c.RepairCtrl.GetRepair)
		gas.GET("/cj", m.RoleMid.IsLoggedIn, c.RepairCtrl.GetRepairDate)
		gas.POST("", m.RoleMid.IsLoggedIn, c.RepairCtrl.CreateNewRepair)
		gas.POST("/approve/:id", m.RoleMid.IsLoggedIn, c.RepairCtrl.ApproveRepair)
		gas.DELETE(":id", m.RoleMid.IsLoggedIn, c.RepairCtrl.DeleteRepair)
